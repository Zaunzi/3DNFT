...
export default function Island({level}:{level:number}) {
 ...
 renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;el.appendChild(renderer.domElement);
 const scene=new THREE.Scene(); const camera=new THREE.PerspectiveCamera(38,1,.1,100);camera.position.set(12,10,14);
 ...
 return <div className="island" ref={host} role="img" aria-label={`Interactive floating farm, upgrade level ${level}. Drag to orbit and pinch to zoom.`}>{error&&<p className="render-error">3D rendering is unavailable on this device. You can still harvest and upgrade below.</p>}</div>
}
