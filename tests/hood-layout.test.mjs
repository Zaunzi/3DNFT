...
test('all buildings and their frontage clear every roadway',()=>{
 for(const b of [...BUILDINGS,...LANDMARKS])for(const road of ROADS)assert.equal(overlaps(b,road,3),false,`${b.id} overlaps road ${road.x},${road.z}`);
...
test('cars can complete every lane and intersection without hitting a building or furnishing',()=>{
 ...
 const buildings=buildNeighborhood(scene,box,()=>new T.Mesh(geometry,material),obstacles,solid);const landmarks=buildLandmarks(scene,box,()=>new T.Mesh(geometry,material),obstacles,solid);
 ...
 buildings.dispose();landmarks.dispose();geometry.dispose();material.dispose();
});
