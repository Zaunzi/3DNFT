...
<main class="oil-app" class:is-embed={embed}>
 ...
 {#if onchain}<form class="oil-token-form" onsubmit={e=>{e.preventDefault();viewParcel()}}><label for="oil-token">Parcel ID</label><div><input id="oil-token" inputmode="numeric" bind:value={tokenInput} disabled={busy}/><button disabled={busy||loading}>Load parcel</button></div></form>{/if}
 <fieldset class="parcel-options"><legend>{onchain?'New parcel size':'Choose your parcel'}</legend>{#each PARCELS as p,i}<label class:selected={size===i}><input type="radio" name="parcel-size" value={i} bind:group={size} disabled={busy}/><span class="parcel-mark">{['S','M','L'][i]}</span><span><b>{p.name} land</b><small>{number(p.min)}–{number(p.max)} OIL / season</small></span><span class="parcel-check">{#if size===i}<Check size={16}/>{/if}</span></label>{/each}</fieldset>
 {#if onchain}<button class="oil-secondary mint-oil" onclick={()=>act('mint')} disabled={!oilConfigured||!$wallet.connected||$wallet.chainId!==84532||busy}>Mint {PARCELS[size].name.toLowerCase()} test parcel <ArrowUpRight size={16}/></button><p class="oil-note">Test faucet · one parcel per wallet · gas required.</p>{#if $wallet.connected&&$wallet.chainId!==84532}<button class="oil-secondary" onclick={changeNetwork} disabled={busy}>Switch to Base Sepolia</button>{/if}{#if !oilConfigured}<p class="oil-warning">OilField v2 address needed. Your existing farm address is preserved.</p>{/if}{#if snapshot}<p class="oil-owner">Owner: {short(snapshot.owner)} {owns?'· You':''} <a href={`/oil/embed/?token=${token}`} target="_blank" rel="noreferrer" aria-label="Open NFT viewer"><ExternalLink size={13}/></a></p>{/if}{/if}
